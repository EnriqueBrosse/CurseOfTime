...
//----------------------------------------------------------------------//
UBTPlayMontage::UBTPlayMontage(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	TimerDelegate = FTimerDelegate::CreateUObject(this, &UBTPlayMontage::OnAnimationTimerDone);
	PreviousAnimationMode = EAnimationMode::AnimationBlueprint;
	bNotifyTick = true;
}

EBTNodeResult::Type UBTPlayMontage::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	if (AnimationToPlay && MyController && MyController->GetPawn())
	{
		...
		if (SkelMesh != nullptr)
		{
			...
			MyCharacter->PlayAnimMontage(AnimationToPlay, AnimPlayRate);
...
EBTNodeResult::Type UBTPlayMontage::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	AAIController* const MyController = OwnerComp.GetAIOwner();
...
FString UBTPlayMontage::GetStaticDescription() const
{
...
void UBTPlayMontage::OnAnimationTimerDone()
{
	if (MyOwnerComp)
	{
		CleanUp(*MyOwnerComp);
...
void UBTPlayMontage::CleanUp(UBehaviorTreeComponent& OwnerComp)
{
...
FName UBTPlayMontage::GetNodeIconName() const
{
